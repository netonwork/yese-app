...
// 模拟观看历史数据
const generateMockHistory = (count: number = 24) => {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      id: `history-${i + 1}`,
      title: `观看历史视频 ${i + 1} - 这是一个很长的标题用来测试显示效果`,
      ...
      watchTime: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
      watchProgress: Math.floor(Math.random() * 100), // 观看进度百分比
...
export const WatchHistoryPage = () => {
  ...
  const totalVideos = 72 // 模拟总数
  const totalPages = Math.ceil(totalVideos / videosPerPage)

  // 生成模拟观看历史数据
  const mockHistoryVideos = useMemo(() => {
    const startIndex = (currentPage - 1) * videosPerPage
    return generateMockHistory(videosPerPage).map((video, index) => ({
      ...video,
      id: `history-${startIndex + index + 1}`,
    }))
  }, [currentPage, videosPerPage])

  const [videos, setVideos] = useState(mockHistoryVideos)

  // 模拟数据加载
  useEffect(() => {
    setLoading(true)
    const timer = setTimeout(() => {
      setVideos(mockHistoryVideos)
      setLoading(false)
    }, 300)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:block flex-1 max-w-2xl mx-8">
              <button
                onClick={() => {
                  console.log('WatchHistoryPage: 搜索按钮被点击，设置 isSearchOpen 为 true')
                ...
                <nav className="space-y-2">
                  {categories.map((category) => (
                    <button
                      key={category.id}
                      onClick={() => {
                        console.log('WatchHistoryPage 桌面端分类按钮:', category.name, category.id)
                        if (category.id === 'home') {
                          // 点击首页，导航到首页
                          navigate('/')
                        } else {
      ...
      <MobileSidebar
        ...
        onCategorySelect={(categoryId) => {
          console.log('WatchHistoryPage: 分类选择:', categoryId)
          if (categoryId === 'home') {
            // 点击首页，导航到首页
            navigate('/')
          } else {
